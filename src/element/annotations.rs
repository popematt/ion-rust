...
impl IonDataOrd for Annotations {
    fn ion_cmp(&self, other: &Self) -> Ordering {
        // Identical tagged pointers are equal without decoding, as in `AnnotationsRepr`'s
        // `PartialEq`: empty vs empty, or a value against itself.
        if self.symbols.ptr == other.symbols.ptr {
            return Ordering::Equal;
        }
        self.symbols.as_slice().ion_cmp(other.symbols.as_slice())
    }
}
...
impl AnnotationsRepr {
    ...
    fn into_boxed_slice(self) -> Box<[Symbol]> {
        let (tag, base) = self.as_tag_and_ptr();
    ...
    fn from_boxed_slice(boxed: Box<[Symbol]>) -> Self {
        match boxed.len() {
            0 => Self::empty(),
            ...
            2 => unsafe {
                Self::from_tag_and_ptr(Self::TAG_TWO, Box::into_raw(boxed).cast::<[Symbol; 2]>())
            },
