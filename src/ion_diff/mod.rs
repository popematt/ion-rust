...
mod diff_tests {
    ...
    #[case::change_container_type(
        "[A, B]",
        "(A B)",
        "value_modified::{path: (), old: [A, B], new: (A B)}"
    )]
    ...
    #[case::duplicate_field_names("{ a: A, a: B }", "{ a: B, a: A }", "")]
    #[case::duplicate_field_names(
        "{ a: A, a: B }",
        "{ a: A, a: C }",
        r"
    ...
    #[case::duplicate_field_names("{ a: A }", "{ a: A, a: A }", "added::{ path: (a), new: A }")]
    ...
    fn test_element_diffs(#[case] old: &str, #[case] new: &str, #[case] diff: &str) {
        ...
        let actual_diff_ion: Vec<Element> = d
            ...
            .map(|x| x.into())
            .collect::<Vec<_>>();
        let actual_diff_ion_for_display: Vec<Element> =
            d.calls.iter().map(|x| x.into()).collect::<Vec<_>>();
