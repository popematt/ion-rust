...
fn cmp_elements(this: &Element, that: &Element) -> Ordering {
    ...
    match this.value() {
        ...
        Value::SExp(this) => cmp_seq_by(
            this.elements(),
            that.as_list().unwrap().elements(),
            cmp_elements,
        ),
        Value::List(this) => cmp_seq_by(
            this.elements(),
            that.as_list().unwrap().elements(),
            cmp_elements,
        ),
...
fn cmp_structs(this: &Struct, that: &Struct) -> Ordering {
    ...
    cmp_seq_by(
        these_fields.iter().map(|x| x),
        those_fields.iter().map(|x| x),
        cmp_fields,
    )
}
...
fn cmp_seq_by<T, R: Deref<Target = T>, I1: IntoIterator<Item = R>, I2: IntoIterator<Item = R>>(
    this: I1,
    that: I2,
    cmp: fn(&T, &T) -> Ordering,
) -> Ordering {
