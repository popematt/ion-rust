...
impl Struct {
    ...
    fn fields_eq(&self, other: &Self, eq: impl Fn(&Element, &Element) -> bool) -> bool {
        for (field_text, self_count) in self.fields.distinct_field_names() {
            let (_, other_count) = match field_text {
            ...
            // Bitvector multiset matching: O(m^2) per field name where m is the repeat count.
            let other_values: Vec<&Element> = match field_text {
                Some(text) => other.fields.get_all(text).collect(),
                None => other.fields.get_all(Symbol::unknown_text()).collect(),
            };
            let mut matched = vec![false; other_values.len()];

            let self_iter = match field_text {
                Some(text) => self.fields.get_all(text),
                None => self.fields.get_all(Symbol::unknown_text()),
            };

            for field_value in self_iter {
                let found = other_values
                    .iter()
                    .enumerate()
