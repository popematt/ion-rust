...
impl<'a> BinaryBuffer<'a> {
    ...
    pub fn read_int(self, length: usize) -> ParseResult<'a, DecodedInt> {
        ...
        // Fast path: fits in i128 (≤16 bytes, or 17-byte i128::MIN)
        if length <= mem::size_of::<u128>() {
            ...
            if let Ok(mut value) = i128::try_from(magnitude) {
                if is_negative {
                    value = value.neg();
                }
                return Ok((
            ...
            // magnitude > i128::MAX — fall through to big path
        }

        // Big path: sign-and-magnitude BE bytes → Int
        cold_path! {{
            let mut magnitude_bytes = int_bytes.to_vec();
            magnitude_bytes[0] &= 0b0111_1111; // clear sign bit

            // Convert BE magnitude to LE
            magnitude_bytes.reverse();

            // Build the Int: construct as positive UInt bytes, then negate if needed
            // We need to go through two's complement. First make a positive Int from the magnitude,
            // then negate.
            // Add a zero byte at the end (MSB in LE) to ensure it's interpreted as positive
            magnitude_bytes.push(0);
            let value = Int::from_signed_bytes_le(&magnitude_bytes);
