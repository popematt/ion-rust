...
fn main() {
    ...
    eprintln!("\n=== Timing ===");
    eprintln!(
        "With drop:    {:.3}s ({:.1} MB/s)",
        per_iter_full.as_secs_f64(),
        data.len() as f64 / per_iter_full.as_secs_f64() / 1_000_000.0
    );
    eprintln!(
        "Without drop: {:.3}s ({:.1} MB/s)",
        per_iter_no_drop.as_secs_f64(),
        data.len() as f64 / per_iter_no_drop.as_secs_f64() / 1_000_000.0
    );
    eprintln!(
        "Drop cost:    {:.3}s ({:.1}%)",
        drop_time.as_secs_f64(),
        drop_time.as_secs_f64() / per_iter_full.as_secs_f64() * 100.0
    );
    ...
    eprintln!("\n=== Binary comparison ===");
    eprintln!(
        "Binary data size: {} bytes ({:.1} MB)",
        binary_data.len(),
        binary_data.len() as f64 / 1_000_000.0
    );
    ...
    eprintln!(
        "Binary with drop: {:.3}s ({:.1} MB/s text-equivalent)",
        per_iter_binary.as_secs_f64(),
        data.len() as f64 / per_iter_binary.as_secs_f64() / 1_000_000.0
    );
    eprintln!(
        "Text/Binary ratio: {:.2}x",
        per_iter_full.as_secs_f64() / per_iter_binary.as_secs_f64()
    );
}
